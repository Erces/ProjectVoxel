...
public class WpnHolder : MonoBehaviour
{
    ...
    public List<WeaponVirti> weapons = new List<WeaponVirti>();
    ...
    void Update()
    {
        ...
        else if (!run && !Input.GetKey(KeyCode.LeftControl))
        {
            ...
            {
                case WeaponVirti.ShotType.Type.Cascate:
                    if (Input.GetButton("Fire1"))
                        Fire();
                    break;
                case WeaponVirti.ShotType.Type.Individual:
    ...
    public void AddWeapon(WeaponVirti wpn)
    {
    ...
    public void EquipWeapon(WeaponVirti wpn)
    {
        if (!wpn || custoMode)
            return;
    ...
    public void UnequipWeapon(WeaponVirti wpn)
    {
        if (!wpn || custoMode)
            return;
        print("unnedquipped");
    ...
    public int IndexOfWpnActual(WeaponVirti wpn)
    {
        for (int i = 0; i < weapons.Count; i++)
            if (weapons[i] == wpn)
                return i;
